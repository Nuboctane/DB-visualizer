...
$rowHeader->append(new Cell('favorieteagent'));
$rowHeader->append(new Cell('bericht'));
 
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $cellId = new Cell($row['userid']);
        $cellNaam = new Cell($row['naam']);
        $cellBeschrijving = new Cell($row['favorieteagent']);
        $cellPrijs = new Cell($row['bericht']);
